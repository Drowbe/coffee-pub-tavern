...
```
<li class="item [done] [conflict] [dragging]" data-id="…" data-kind="stop|stay|journey|note|ref"
    data-cat="things|food|stay|travel|other"  data-span="start|middle|end"  data-drop="before|after|into">
...
```
`data-cat` is on stops, notes and the module's own kinds (a `ref` has none). `data-span` is only on a `stay`. State classes: `.done` (crossed out), `.conflict`, `.dragging`.

**Slots inside `.item-main`** (all optional except the title; the script fills only the ones it has):
- `h3.item-title[data-slot=title]`
- `p.item-sub[data-slot=place]` (an icon and an address or place) or `p.item-sub[data-slot=body]` (a note's text, a ref's subtitle)
...
**By kind**
- `stop`: the slots above. `data-cat` is one of `things`, `food`, `other`.
...
The item editor (a dialog over the page, as the To-do's), the Bookings view, and the map are designed when their phase starts.
