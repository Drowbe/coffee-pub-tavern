...
- Icons are `<span class="ic" data-icon="utensils">` holding the inline SVG from `host.ui.icon('utensils')`. The stylesheet sizes `.ic svg`; the script never sizes an icon.
...
The way to add is the host's bottom bar, the same quick-add the To-do, Polls and Calendar have (`host.bar.set` with a `quickadd` item). When the host has a bar, every day's `form.add-row` gets the class `hosted` (the stylesheet hides it). When `bar.set` is refused, or a host has no bar, `hosted` is removed and each day keeps its own row. Text typed in the bar becomes a stop on the **current day**: the day named in the text if it finds one inside the trip, else the day whose chip is `.current` (the one in view), else today if it is on the trip, else the first day. An empty submit opens the editor; with no trip yet it opens the trip form.
...
One popover the script positions under the `.item-menu` button that opened it (`position: absolute` inside `.app`; it sets `top` and `left` as its only inline style, the one exception to "no inline styles", since a position cannot be a class). Entries, each `button[data-action]`: `edit`, `earlier`, `later`, `to-line` ("Back to the line", for an item on a day), `delete` (`.danger`), and a `label` with `select#menu-day` ("Move to": every day and every joint of the line in line order, "Before the first day", "Thu, Oct 1", "Between Oct 1 and Oct 2"... "After the last day"; option values are `d:<date>` for a day and `j:<after>` for a joint, `j:` the head). The script fills `#menu-day` and shows or hides entries by kind and position. The popover opens under its button, or above it when there is no room below. `[hidden]` closes it; a click elsewhere or Escape closes it.
...
**The day header** (`tpl-day-head`): `[data-slot=daynum]` (the day of the month, large), `[data-slot=daymonth]` ("Sat · Oct"), `[data-slot=position]` ("Day 2 of 7") and `[data-slot=summary]`, one line the script builds: the first and last time, how many stops, and the total time getting around, e.g. "10:05 – 23:30 · 6 stops · 1 h 24 min getting around". At its right, `button.menu-btn[data-action=day-menu]` (the one "..." icon, `ellipsis-vertical`; hidden for a viewer who cannot edit) opens `host.menu.show` with everything that can be added to that day: every kind the editor's tiles offer (a flight, a train, a ferry, a bus, a rental car; a stay; a restaurant, a café, a bar, a sight, a museum, a tour, a show; a note) and the time blocks from the settings, each opening the editor on that day with the kind chosen. The + on a joint (`gap-add`) opens the same menu for its place on the line, with the between-days marker types in place of the time blocks (a marker is added at once). The type-to-add row (and the host's bar) is the fast path; this is the plain one. On a day the menu also has a **Mark the day** group: one entry per marker type that adds a marker with no time at once (see below), ahead of the timed ones ("Add free time at a time", which open the editor).

**Day tags.** A marker (`kind: block`) with no time is about the whole day ("Travel day", "Free day": two default marker types beside the timed ones). It is not a row: the header's `div.day-tags` (hidden when there are none) holds one `button.daytag[data-action=move-menu][data-id][data-type]` per such marker (`tpl-day-tag`: the type's icon and the title, `--marker` its colour, the note as its tooltip), and clicking the tag opens the item menu for it, like a row's "...". A viewer who cannot edit sees the tags disabled. Give the marker a time and it becomes a row in the day again.

**More.** What a card's face has no place for is folded under it: `details.card-more` (`tpl-card-more`) appended last inside the `article.card`, a `summary` reading "More" ("Less" when open) and a `dl` of label/value pairs -- only the fields the item has and the card's template shows no slot for (the note, who is on it, the booking reference, the terminal, the platform, the carriage, the seat, the class, the pick-up and drop-off, the room, the guests, the reservation name, the address, and the cost with who paid). Absent when there is nothing to fold. A card someone opened stays open through redraws (the script keeps the ids). Not on a pointer's card or a between-days marker.
...
**Empty days.** A toggle in the toolbar (`button.tool-empty[data-action=toggle-empty][aria-pressed]`, icon `eye-slash`, tooltip "Hide empty days" or "Show empty days") sits in `.actions` beside the pencil. A day with no items (markers do not count) gets `.is-empty`, and so does its chip in the strip. While the toggle is on, `.app.hide-empty` hides both, and each run of hidden days is one `div.gap` badge on the line (see The line, and the days-between badge); `p.emptynote` is only a fallback where a run cannot be shown. If every day is empty nothing is hidden. The choice is remembered for the person, and the default is off.

### Markers: one pill, one colour (0.7.0; supersedes the earlier coloured bands and the draft time blocks)

Reference rendering: `design/markers.html` (`?state=normal|edge|hidden|blocks`, `?theme=light`, `?w=390`). The author's sketch: a rounded pill in two parts, an icon cell and a label cell, sitting on the line that joins the boxes.

**The pill** (`.markerpill`, `.mk-icon` with an icon, `.mk-label` with `b`, and `.sub` or `.mk-len` and `.mk-note`): one colour, `--marker`, gives everything. The border is that colour, the icon cell is that colour, the label cell is that colour lightened into the card (with a fine diagonal hatch), and the icon is white with a little see-through so it takes a touch of the colour it sits on. Until a colour is chosen in the settings each type has a default from the card families' hues, turned from the accent, so a theme still rules the look.
...
**On the line.** The icon cell of a marker (plan-wide, or a time block in a day) sits on the line: its centre is the rail's (`margin-left` on the pill pulls it left over the rail column, and the rail's dot is hidden), the same as every other row's, on a narrow pane too — nothing about the pill changes where the rail sits. A time block's time stays in `.when`, to its left (above it on a narrow pane, where the icon cell is a little smaller). A plan-wide marker's date is different: it is also in `.when` (`[data-slot=time]`) for a wide pane, but on a narrow one `.when` hides and `span.mk-date[data-slot=mdate]` (the same date, filled alongside it) shows inside the pill's label instead, before the title — a plan-wide marker's pill fills the whole row on a narrow pane, so stacking the date in its own line above, the way `.when` does everywhere else, would leave it stranded; a time block's pill does not, so it keeps the usual place.
...
### The line: items between the days

Every item is in one of two places (the model enforces it, `cleanItem`): **on a day** (`date`, an optional `time`, then `order` among the day's untimed items) or **on the line at a joint** (`after`: the date of the day the joint follows, `""` for the head of the line before the first day; then `order`). An item with neither shows on its pointed-at item's day when it has one, else at the head. So an idea with no day yet, a research card placed "near day 3", a poll between two days and a between-days marker are all the same thing: an item at a joint. A marker between the days ("Rest days", "Travel") is the one kind, `lane`, that is only ever on the line: a marker type, an optional label and note, no time.

- Markup: `ol.timeline.between[data-after]` between the day blocks (`data-after=""` at the head, between the "Planning starts" marker and the first day), holding the items at that joint in their order: a marker as `li.row.entry.marker.lane[data-kind=lane]` from `tpl-row-lane` (the same two-part pill as a time block, its `⋯` on its right edge), anything else as the row and card it would be in a day, with an empty `.when` (a time means nothing on the line). `data-type`, `--marker`, the icon and the title are set as for a time block.
- The item menu for anything on the line: **Earlier** and **Later** (a swap with the neighbour at the joint; at either end, a hop to the next joint), **Move to** (any day or joint; a marker sees only the joints), **Remove** or **Delete**. A marker also has **Change type**. Edit opens the editor; its **Where** field (`select#f-date`, the same options and values as Move to) is how a new item is placed, and a marker's shows only the joints.
- **The joint** (`div.joint`, `tpl-joint`): a small + on the line where two days meet (and before the first and after the last), quiet until the pointer is over the days (always faint on a touch screen). It opens the add menu for that place (see the day header above). The badge for hidden days does the same for its own place; a hidden day's joint draws its items after the badge and has no + of its own.
- Reference: `design/markers.html?state=lanes`.

**Dragging onto the line** (the author: "dragging is key"; everything stays possible without it, through the menu, the editor's Where and the joint +). While anything is dragged over the plan (one of its own items by its body or a marker by its pill, not the `⋯` or the rail; or another module's item or card), `.days.dragging-line` is set and every joint opens into a dashed line with a + on it; the joint or day under the pointer is `.drop-target` (a joint: a solid accent line and a filled +; a list of items at a joint: a dashed outline; a day: as before), the plan's own item being moved is `li.dragging`, and on another item at the same joint or day `data-drop=before|after` draws an insertion line. Letting go moves the plan's own item there (`after` becomes the joint, `date` null; or the day, among its untimed items); another module's item is offered "Put it here, between Oct 1 and Oct 2" through the shared drop menu. A marker's drag takes the nearest joint wherever the pointer is. The rail handle's drag (the browser's own) does the same. Reference: `design/markers.html?state=dragging`.
...
**The badge** (`div.gap`, `tpl-gap`) stands in where empty days are hidden: one badge for each run of hidden days, on the line. A circle holds how many days (`[data-slot=count]`, tooltip "3 days between"); a + beside it (`button.gapadd[data-action=gap-add]`) opens the add menu (`host.menu.show`) for the joint before its first hidden day: everything that can be added there, the between-days marker types (each icon in its colour), and "Show these days". `data-from` and `data-to` are the first and last hidden day. It replaces the "N empty days hidden" note, and it stands in for each hidden day's own joint too: a hidden day gets none of its own (they would only pile up on the badge, at the same place on the line, for no day a person can see), so a run of any length is ever only the one badge and the ordinary joints on either side of it.
...
**Round trips** (0.7.29). A round trip is two journey items: the outbound, and the return with `legOf: <the outbound's id>` (`cleanItem` keeps `legOf` only on a journey that is not a car and not its own id; the outbound holds `confirm`, `cost` and `paidBy`, so a return is saved without them, and a return found with its outbound shows the outbound's and is left out of Money by `costItems`. A leg whose outbound is not found keeps and shows its own). The model reads the pair with `returnOf(items, id)` and `outboundOf(items, item)` (`plan.returnFor(id)`, `plan.outboundFor(item)`); a return whose outbound is gone, or a second return for one outbound (two people at once; the first by `order` is the return), reads as a one-way journey. `plan.addRoundTrip(outbound, back)` adds both, `plan.removeLeg(id, both)` deletes both legs or one (only the outbound: the return becomes one-way and takes the booking reference, cost and payer), `plan.extraReturns(id)` lists the second returns. More legs can use the same field later.
- **The editor.** `div.fieldgroup.roundtrip` (`data-types` every journey but `car`) holds `input#f-roundtrip[type=checkbox][role=switch]` ("Round trip") and `div.return-leg#f-return` (`[hidden]` while the switch is off): the title "Return", `f-back-title` (optional; empty is "Flight to London", or "Return: <the outbound's title>"), `f-back-date` (the same options as `f-date`, labelled "Return day"), `f-back-time`, `f-back-hours` + `f-back-minutes`, `#f-back-arrives`, `f-back-fromCode` + `f-back-toCode` (flight), `f-back-from` + `f-back-to`, `f-back-number` (not taxi, ride share), `f-back-seat` (flight, train). The labels `f-back-time-label`, `f-back-minutes-label`, `f-back-number-label`, `f-back-from-label` and `f-back-to-label` take the same words as the outbound's. Turning the switch on for a new return fills its ends from the outbound's, swapped, and its day with the trip's last day. `p.hint#f-booking-both` ("The reference, cost and payer cover both legs.") shows under Booking while the switch is on. Either leg opens this one editor with both legs (the outbound's fields are the main ones); saving writes each leg that changed with its own version, and a conflict shows on that leg's row as before. Turning the switch off on an existing pair asks through `host.menu.show` anchored to the switch ("Remove the return leg", "Keep it"); the return is deleted on save, and `p.hint#f-roundtrip-note` says so. `p.hint#f-extra` with `button#f-extra-delete` ("Delete the second one") shows when two returns were saved at once.
- **The cards.** Each leg is its own row on its own day, with `data-leg="outbound|return"` on the row. `div.card-roundtrip` (`tpl-card-roundtrip`), inside the `article.card` just before `.card-more`, holds the `right-left` icon, `[data-slot=roundtrip-text]` ("Round trip · Return Fri, Oct 9" on the outbound, "Round trip · Outbound Sat, Oct 3" on the return, "... not on a day yet" when the other leg has none) and `span.roundtrip-warn[data-slot=roundtrip-warn]` ("Return is before the outbound", in `--danger`; hidden otherwise; the move is allowed). The return's card shows the outbound's booking reference and, under More, its cost and payer.
- **Bookings** has one row for a round trip, the outbound's: both days and the route both ways ("Sat, Oct 3 09:30 · Fri, Oct 9 18:00 · LHR ⇄ JFK"). **Money** is unchanged: the return has no cost of its own, so the booking is counted once.
- **Delete.** Deleting either leg (the item menu's Delete, or the editor's `#f-delete`) asks through `host.menu.show` anchored to that button: "Delete both legs" (danger), "Delete only this leg" (with a hint: "The outbound stays." or "The return stays, with the booking details."), "Cancel". Anything else keeps its "Really delete?" step.
