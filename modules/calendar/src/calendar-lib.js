  ...
  // Times on the server's clock (tavern.locale().clock): the page and the widget call setClock once they know it; 12-hour until then.
  let hour12 = true;
  const setClock = (clock) => { hour12 = clock !== '24'; };
  const timeText = (d) => d.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', hour12 });
