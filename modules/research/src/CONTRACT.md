...
- **Drop onto the pane:** while something from another module is dragged over Research, `.rs.drop-target` draws a dashed accent frame round the pane and `#drop-hint` ("Drop to start a note about it") appears above the bottom bar. Dropping opens the shared drop menu (`host.refs.dropMenu`, see `api-module-sdk.md`, "Dragging"): "Start a note about it" is this module's own offer (a note with the item's title, linked to it; a card carried by the drag keeps its text as the body), followed by whatever the modules around offer for an item of that kind.
