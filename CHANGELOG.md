...
- The Calendar module (`modules/calendar`, built zip in `modules/dist`): a month grid and an upcoming list, events with reminders, for the server and for each room, updating live. `tools/build-module.mjs` builds a module zip from a source folder.
