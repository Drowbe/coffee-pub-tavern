...
- `tavern.locale()` is `{ language, clock, currency }`: how the server shows these (Manage > Settings > Language, time and money; the defaults `en`, `12`, `USD` until the handshake answers). Apply them with `tavern.util.time("22:30")` ("10:30 PM" on the 12-hour clock, "22:30" on the 24-hour one; a stored time is always HH:MM), `tavern.util.hour12()` (for `toLocaleString`'s `hour12` where you show a Date) and `tavern.util.money(amount, currency?)` (the server's currency unless one is given). Never format a time or an amount your own way: the person chose these once, for everything.
