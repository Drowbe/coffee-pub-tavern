...
- `settings` lists up to 20 settings the module offers people, each `{ key, label, help?, type, scope, default, ... }`: `type` is `boolean`, `choice` (with `options: [{ value, label }]`, two to twelve), `number` (`min`, `max`) or `text` (`maxLength`, up to 200); `scope` says who chooses it: `server` (an admin, for everyone), `room` (an admin or a room's moderators, for that room) or `person` (each person for themselves). A setting holds plain data, never a secret. Tavern draws the forms and keeps the values; the module reads them (see [api-module-sdk](api-module-sdk.md)).
