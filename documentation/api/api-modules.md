...
- `settings` lists up to 20 settings the module offers people, each `{ key, label, help?, type, scope, default, ... }`: `type` is `boolean`, `choice` (with `options: [{ value, label }]`, two to twelve), `number` (`min`, `max`) or `text` (`maxLength`, up to 200), `url` (empty, or an http or https address up to 500 characters) or `file` (the name of a file an admin placed for the module, see below; server scope only; `folder`: where the files go inside modules/<id>/, lowercase letters, digits and dashes, default `files`, never `versions`); `scope` says who chooses it: `server` (an admin, for everyone), `room` (an admin or a room's moderators, for that room) or `person` (each person for themselves). A setting holds plain data, never a secret. Tavern draws the forms and keeps the values; the module reads them (see [api-module-sdk](api-module-sdk.md)).
