...
| Right | `.nav-right` (`.links`) | the system's actions and information, three groups with a divider between: you (`#whoami-link`), the system (the light or dark switch `theme-mode-switch`, not on the host console; Manage `#admin-link`; Install `#install-link`), the session (Call settings in a space, the time `#topbar-clock` on the server's clock setting, Sign out `#logout-link`) |
