...
3. Maps needs Places, so install and enable Places first (its row in the list says so). Then on the Modules tab, choose **Install** beside Maps under **Available with this server**, then **Approve and enable**. It asks to link to other modules' items and to ask other modules to do things.
4. Tick **Available in every space**, or tick it per room.
