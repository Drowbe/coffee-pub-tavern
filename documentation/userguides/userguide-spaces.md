...
with its members and a green dot on those in it right now, and joins one. Owners may join any space.
...
Every space card has a **Join** button, an icon button to the right of it that joins straight into a
pop-out window instead of the page, and (for owners) an **Edit** button. If a space has a launch link,
its icon button sits there too. A space you are still connected to offers **Rejoin** instead of Join.
...
The **Who's around** strip shows everyone who is online: signed in with Magpie open in a page you are looking at, whether or not they are in a space. Each person shows the space they are in, or "online", and a video mark while they are in the call. Beside everyone but you is a **people arrows** button: click it to invite them to a private conversation. It makes a private aside for the two of you, off the record like a pull-aside, and takes you into it; they get a notice wherever they have Magpie open with **Join** and **Decline**. Join takes them straight into the aside. The invitation lasts a couple of minutes, and the aside goes away when nobody is left in it. A person who is not online cannot be invited, and you need the permission to start a private conversation (and the environment must allow them).

## Join with

The button with sliders on a space card, next to Join, chooses which modules the space opens with: the
conference, the chat, and any other module the space has on. A person can join with only the chat, or the chat
and the Calendar, and still be in the space and online for everyone else. The choice is remembered for
that space in this browser and is the same list the space updates itself as you open and close modules, so
the next join opens what you had open last time. A space you have not used before opens with the
...
## Add and set up a space

The **Spaces** tab on the Manage page holds the **Lobby**, which everyone belongs to and which cannot
be deleted, plus any spaces an owner adds. It is a roster: click a space to open its own page. The
...
- **Space**: its name, description and picture, its **Profile**, a **Launch link** with its icon, and
  whether guests are allowed. **Delete space** is at the bottom.
- **Members**: who belongs to the space, the invite tools, and the guest link. Toggling a member saves
  straight away.

**Profile** picks which picture sets the space uses. **Roleplaying** (the default) offers both
Participant and Character images. **Participants** offers Participant images only. **Characters**
offers Character images only. It decides which sections appear on each member's profile for that space.
