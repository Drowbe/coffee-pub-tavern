...
- **Time blocks** are the other kind of marker: things that happen inside a day and have no place, such as free time, rest, a buffer, a meet-up or a leave-by time. Add one from the editor's **Time** group of tiles (a label is optional; the type names it), with a time, a length (not for a meet-up or leave-by) and a note. They are drawn as the same coloured pill, sit among the day's items in time order, and are edited, moved and removed like any item. They are not stops and do not count in the day's summary.
- **Markers between the days** are markers that are not in any day but on the plan's line between the day blocks ("lunch break", "travel day"). Add one with the small **+** that appears on the line where two days meet, before the first day and after the last, or from a hidden-days badge; choose the type. It has a type, an optional label and a note, and no time. Its menu (the three dots) can change its type, move it to an earlier or later joint (**Earlier**, **Later**, **Move to** "After Day 4") and remove it; you can also drag it by its pill to another joint or among the markers already at one. They are not in any day, so they do not count as something planned.
