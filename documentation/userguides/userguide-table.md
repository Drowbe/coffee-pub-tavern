...
The conference has the same titlebar as the chat and every module: the same buttons to float it over the page (and dock it again), to open it in a window of its own, and to close it, which leaves the call. In its own window the titlebar has no close: closing that window brings the call back into the page where it was, and **Hang up** on the toolbar is how you leave. Chat opens as a column beside the video. Its header has the same buttons as a module's: one to make it a floating panel you can drag and resize (and a matching button to dock it again), one to open it in a window of its own, and the x to close it. A floating panel also has a **Snap to a grid** button: on, the panel sits in the cells of a grid over the call (you see the grid while you drag), moving and resizing a cell at a time and keeping its place when the window changes size; off, it floats freely, as before. Each panel remembers its own choice. The room bar (beside Full screen) has the same switch for the whole stage: on, every panel that can float is put on the grid -- docked ones float first -- and any you open later comes up floating and snapped; a slider beside it sets how fine the grid is (you see the grid while you slide). Off, the panels that were docked when you switched it on dock again, and the rest float freely. The room remembers both. The Calendar and other modules open as more columns after the chat, and the video always keeps some room. If you pop the whole call out, chat and modules come with it. Close a pane's own window and the pane comes back into the page, docked or floating as it was before, with whatever it held untouched.
