...
- **On the dashboard.** A module with a widget adds a card to the dashboard on the spaces page, and its heading opens the module's own page. A module with its own page and no widget adds a link to the header instead, for everyone who may see it. Opened from inside a call a page appears over the call, so the call keeps running.
- **In a call.** A module that opens on a space's canvas gets a switch in the space bar under the header, beside the conference and the chat. The space bar is the one place to show or hide them, and each switch shows an unread count for the chat and for module notifications. Switch one on and it opens as a column beside the video and the chat when the module supports that (video, chat, then the module), or floating over the call. Drag a docked column's left edge to change its width. The buttons in a module's header switch it between docked and floating, open it in a window of its own, or close it; a floating module's **Snap to a grid** button makes it tile into a grid over the call instead of floating freely (see [The call](userguide-call.md)). Several can be open at once, and each remembers how you had it. On a narrow window, such as a phone, one module fills the canvas at a time, and the space bar becomes a row of tabs at the bottom. If you pop the whole call out into its own window, the modules come with it.
- **Notifications.** A module can notify you, for example a reminder. It appears as a toast, and as a number on the module's dashboard card heading (or its header link, for a module without a widget) and on its switch in the space bar until you open the module.

## Turn a module on in spaces

A module that opens on a space's canvas is off in every space until you turn it on. Either tick **Available in every space** on its card here, or open a space's own page from the Spaces tab, go to its **Modules** tab (it appears when there are modules to set) and tick the module. The Modules section of a space only lists modules that are enabled.
