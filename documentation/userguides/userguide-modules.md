...
- **Source:** **Managed** uses the service the host runs for every environment, with the host's own key (set on the host console, or by the server's `AI_PROVIDER`, `AI_MODEL`, `AI_ADDRESS` and `AI_KEY` environment variables on a single server); nothing else needs setting here, and the option says which company and model that is, or that the host offers none. **Custom** is a service and key of this environment's own, with the fields below. Changing the source switches AI off until you enable it again, since a different company would receive what people select. The monthly allowance and this month's use are this environment's own either way.
...
- **Key** (custom): kept on the server and never shown again. The panel says only whether one is set. **Set a key** or **Replace the key** takes a new one; **Remove the key** deletes it. The host's own key, for the managed service, is on the host console, where it says so if it comes from the server's environment (`AI_KEY`).
