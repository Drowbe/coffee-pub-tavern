...
- Module interoperability, next steps beyond refs, opening and backlinks (which are built): modules reacting to each other's events, and asking each other to create things, both declared in module.json and carried by Tavern with no module named in Tavern's code. The design sketch is in plans/plan-modules.md. Also verify in a real browser: dragging an event or a poll from one pane onto a task in another, and between windows.
