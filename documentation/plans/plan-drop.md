...
3. **A plain `ref` input takes the dropped item.** The typed form (`ref:module:kind`) still narrows an action to one kind; the plain form means "any item", which is what most actions want. An action is offered only when the dropped item filled at least one of its inputs (typed or plain), so an action unrelated to the item is never offered just because a date was under the pointer.
...
9. **An action of the item's own module, taking it only as any `ref`, is left out.** It would make something of the module's own item somewhere else (a task from a task). Taking it by its exact kind (`ref:todo:task`, "link this task to...", "set its due date") is what a drop means, and stays.
...
**As a provider**, every action keeps its declared inputs; the retrofit checks each one against the vocabulary (a `ref?` on `createEvent`, `createTask`, `addStop`, `saveNote`, `saveLink`, `askAssistant` is what makes them show up) and that each creates a link back where it makes one.
