...
## Managed AI (decided 2026-09-23)

**What the author wants.** The AI service's key is the host's, above the environments: the host operator sets one managed service, and each environment chooses **managed** (the host's service) or **custom** (its own provider, address, model and key). Asked as "the setting for the API key for AI should be at the [host] level; then they choose managed (the [host's]) or custom (their own)".

**The contract.** Two halves: the server (the Server Development session) and the pages and docs (this session).

- **The host's managed service** lives in `host.json` (`ai: { provider, address, model, key }`), set on the host console: `GET /api/host/ai` answers `{ ai: { provider, address, model, keySet, keyFromEnvironment } }` (never the key); `PUT /api/host/ai` takes `{ provider, address, model, key?, clearKey? }` with the same validation as an environment's own setting (`provider` `none` means no managed service is offered); `POST /api/host/ai/models` lists a provider's models for the console the way `POST /api/ai/models` does for an environment. All host-admin only.
- **Environment variables seed the managed service.** `AI_PROVIDER` (`openai`, `anthropic` or `compatible`), `AI_ADDRESS`, `AI_MODEL` and `AI_KEY` describe the managed service when the host has none saved (and are the only way on a single-environment install, which has no console); `keyFromEnvironment` on the host view says so. `AI_KEY` used to be an environment's own key; it is the host's now.
- **An environment chooses.** `ai.json` gains `source: 'managed' | 'custom'`. `GET /api/ai` answers `{ ai: { source, managed: { available, provider, model }, provider, address, model, monthlyTokens, keySet, keyFromEnvironment, enabled }, usage, dependents }`, where `managed` describes the host's offer (no key, no address). `PUT /api/ai` accepts `source` beside the custom fields; `managed` is refused when the host offers none; changing the source switches AI off (a different company receives what people select) until the admin enables it again on the Modules tab. With `source: 'managed'`, calls use the host's provider, address, model and key; the environment's own monthly allowance and usage still apply. `ready()` with `managed` needs the host's service set and the environment's enable.
- **Default and migration.** A fresh environment starts `managed` when the host offers a service, else `custom`. An existing `ai.json` with a saved provider and model but no saved key, which worked only through `AI_KEY`, becomes `source: 'managed'` on first start, its provider, address and model seeding the host's managed service if the host has none saved yet, so nothing that worked stops working. An `ai.json` with its own saved key stays `custom`.
- **Pages.** The environment's AI configuration page starts with the choice (Managed: "this host's service, <provider>, <model>", or Custom, with the fields as today); the host console gains a Managed AI panel (provider, address, model, key write-only, models listed from the company). The plan's `aiCallsPerMonth` cap (phase 3) applies to both sources.

## Shared files: the host's map (decided 2026-09-23)

**What the author wants.** "Same with maps": the map files are the host's, not each environment's ("I do not want to deal with the storage per environment yet"), only the host admin puts files there and cuts regions ("only the admin can slice"), and in phase 1 an environment's only choice is the host's map (no web address, no files of its own).

**The contract.**

- **Manifest.** A `files` setting may declare `"shared": "host"`: its folder is the host's, one for every environment, and a `regionSource` on that folder cuts into it. Maps declares `map` that way; its `mapSource` and `mapUrl` settings go (phase 1: the host's map or nothing), `worldSource` stays as the region source's address and is the host's when the folder is shared.
- **Where the files are.** With `BASE_DOMAIN`: `DATA_DIR/shared/<module id>/<folder>/`. Without it: `DATA_DIR/modules/<module id>/<folder>/`, exactly where they are today, so a self-hosted install moves nothing and its admin (who is the operator) keeps the files and region cutting on the module's configuration page. On the first start with `BASE_DOMAIN` where the shared folder does not exist yet and exactly one environment has files in its own folder for it, those files move to the shared folder (the migrated environment's map keeps working); otherwise the folder starts empty and the host admin copies files in or cuts a region.
- **The environment side.** `GET /api/modules/:id/settings` marks a shared setting `shared: true` and lists the host's files as `available`; `settings/values` answers it with every host file (nothing to tick); `PUT` ignores it; `GET /api/modules/:id/files/:name` serves from the host's folder; the environment's region-cut routes and `DELETE .../files/:name` answer 403 "managed by the host" for a shared folder, and `worldSource` is not the environment's to set (marked `shared: true` too).
- **The host side** (host admin, `hostRouter`): `GET /api/host/shared` -> `{ folders: [{ module, name, folder, files: [{ name, size, zoom? }], address, exists, cutting }] }` for every bundled module with a shared folder; `PUT /api/host/shared/:module/:folder` `{ address }` keeps the region source's address in `host.json`; `DELETE /api/host/shared/:module/:folder/files/:name`; and the region cut as it is on the environment routes today, re-mounted: `GET .../region-cut/find?q=`, `POST .../region-cut/estimate`, `POST .../region-cut`, `GET .../region-cut/:jobId/stream`. The place search a find needs comes from whichever environment has one configured, else none (the console says so).
- **Pages.** The host console gains a **Maps** panel (the files with size and zoom, delete, the world address, find, estimate and cut with progress: the module configuration page's own region-cut UI, moved); an environment's Maps configuration shows the host's files read-only under "Provided by the host".

## Managed AI, per company (decided 2026-09-23, replacing the single managed service above)

**What the author wants.** The operator only ever sets keys: "we have never set anything but the key, the rest is behind the scenes. Why not let the [host] offer both: `AI_ANTHROPIC_KEY`, `AI_OPENAI_KEY`; then if set you do the rest and offer them." So the host offers each company whose key is present, with a model chosen for it by default, and an environment picks which managed company to use, or custom.

**The contract.**

- **The host's offer is per company.** `host.json`'s `ai` becomes `{ openai: { model, key }, anthropic: { model, key }, compatible: { address, model, key } }`, any subset. `AI_OPENAI_KEY` and `AI_ANTHROPIC_KEY` are that company's key when none is saved (`keyFromEnvironment` per company). A company is **offered** when it has a key (or, for `compatible`, an address) and a model. With a key and no saved model, the model is the built-in default for that company (`DEFAULT_MODELS` in `server/ai.js`, one current, inexpensive model per company, chosen by the server session and named in the docs), so a key alone is enough; the console can change it. The earlier `AI_PROVIDER`, `AI_ADDRESS`, `AI_MODEL` and `AI_KEY` keep working as the one-company form of the same thing (they fill that company's slot), and the migration seed from an old `ai.json` still lands in that company's slot.
- **Host API.** `GET /api/host/ai` -> `{ services: [{ provider, model, address, keySet, keyFromEnvironment, offered }] }`, one entry each for `openai`, `anthropic` and `compatible`, never a key. `PUT /api/host/ai` takes `{ provider, model?, address?, key?, clearKey? }` for one company at a time, with the validation the environment's custom slot has. `POST /api/host/ai/models` unchanged (`{ provider, address, key? }`, the saved key of that company as the fallback).
- **The environment's choice.** `ai.json` keeps `source: 'managed' | 'custom'` and gains `managedProvider`. `GET /api/ai` answers `managed: { available, services: [{ provider, model }] }` (the offered companies) and `managedProvider`; `PUT /api/ai` takes `{ source: 'managed', managedProvider }` (refused when that company is not offered) or the custom fields with `source: 'custom'`; a change of source or of managed company switches `enabled` off. A fresh environment starts managed on the first offered company, in the order `openai`, `anthropic`, `compatible`; none offered means `custom`. If the chosen company stops being offered, the environment's effective provider is `none` until an admin picks again.
- **Pages.** The console's Managed AI panel is one row per company: the key (set by the environment, or Set, Replace, Remove), the model listed from the company with the default preselected, the address for Other, Save per row, and whether it is offered. The environment's Source choice lists one Managed entry per offered company ("Managed: OpenAI, gpt-...") and Custom.

## Phases 2 to 5 in detail (decided 2026-09-23, the author asleep: "you are the boss")

Built to one contract by two sessions: the server (the Server Development session, `server/*`) and the pages and docs (the UI session, `public/*`, `documentation/*`). Where the earlier decisions left a choice, the choice made here is named.

### Phase 2: the owner role and the split

- **The owner is the environment's admin.** No new stored role: inside an environment `role: 'admin'` is the owner, and every page says **Owner** for it when the host has environments (`BASE_DOMAIN` set) and **Admin** otherwise. The host admin's own cross sign-in user (`hostAdmin: true`) is not an owner. `GET /api/me` gains `environment: { hosted, slug, name, owner, hostAdmin }` (`hosted` true when `BASE_DOMAIN` is set; `owner` true for an admin of the environment; `hostAdmin` true for the host's own cross sign-in), and `branding()` gains `hosted`.
- **What Manage hides from an owner** (shown only when `!hosted` or the viewer is the host admin): uploading a module zip and "Run in the page" (the host's trust decision), the Font Awesome Pro note, and the whole-server facts that belong to the host. Everything else stays the owner's: Server, Theme, Spaces, Roles, Users, Modules (install and enable among the entitled bundled ones, configure), About. The Roles grid's fixed column reads Owner.
- **The environment's own panel.** `GET /api/environment` (admins of the environment) answers `{ slug, name, status, pastDueSince, graceEndsAt, plan: { name, modules, members, storageBytes, aiCallsPerMonth, calls }, usage: { members, storageBytes, aiCallsThisMonth, callsNow } }`. Manage's Server tab gains an **Environment** panel: the plan's name, each cap with its use as a bar, the past-due banner (also on every page for owners while `status === 'pastDue'`: "Payment is overdue; the environment goes to the free plan on <date>"), **Download a copy** (`GET /api/environment/export`, the tenant's zip, the same the console makes) and **Ask for deletion** (`POST /api/environment/delete-request` with `{ reason? }`, which marks the registry entry `deleteRequestedAt` and shows on the console; `DELETE` withdraws it). Without `BASE_DOMAIN` the panel says the server is its own and shows nothing else.

### Phase 3: the caps, enforced at the seam

Each cap is `null` for none. Over a cap, the one thing stops with a 403 and a plain sentence; everything else keeps running. The registry counts usage where it is cheap and measures where it must:

- **members**: the count of user accounts (guests never count). Refused: creating a user on Manage, self-registration, accepting an invite. Message: "This environment is at its limit of N members."
- **storageBytes**: the tenant directory's size, measured at most once a minute and cached on the registry entry (`usage.storageBytes`, `usage.measuredAt`). Refused: module uploads, profile and space pictures, site images. Message: "This environment has used its N GB of storage."
- **aiCallsPerMonth**: `Ai.record` counts calls per month on the registry entry (`usage.aiMonth`, `usage.aiCalls`), managed or custom. Refused: `host.ai.ask` past the cap. Message: "This environment has used its N AI calls for this month."
- **modules**: `'all'` or a list of module ids; installing or enabling a module not on the list is refused ("This environment's plan does not include <name>"), and the Modules tab's Available list marks them "Not in your plan". A module already on when the plan shrinks stays on until turned off, then cannot be turned on again.
- **calls** (phase 4): the number of this tenant's spaces with a live call (LiveKit rooms with the slug's prefix that have a participant in the call), asked from LiveKit at join time; a join into a space with no call yet is refused at the cap ("This environment's plan allows N calls at once; one is running in <space>"), joining a call already running is never refused.

### Phase 5: self-serve, plans and billing

- **The plan catalog is the host's.** `host.json` gains `plans: { free: { name, caps }, ...named plans }`, `free` always present; the console's new **Plans** panel edits them (name, the five caps), and a tenant's `plan` gains `name` (the catalog key) beside its caps, which are copied from the catalog at assignment and may be adjusted per tenant. `GET /api/product` gains `plans: [{ id, name, caps }]` for the landing page's Plans section and the sign-up form. `GET /api/host/settings` gains `plans`; `PUT /api/host/plans` replaces the catalog.
- **Sign-up at the bare base domain.** `POST /api/product/signup` with `{ slug, name, owner: { login, displayName, password }, plan? }` creates the tenant on the `free` plan (any other named plan needs billing, so the form offers it only as the plan to move to afterwards) and answers `{ url }` (the environment's address); refused with the registry's own messages for a taken or reserved slug. Rate-limited to five an hour per address. `SIGNUP` env var: `on` (default with `BASE_DOMAIN`), `off` (the form says to write to the contact address instead).
- **Billing is a webhook, provider-agnostic.** `POST /api/host/billing` carries a JSON body `{ slug, plan, event }` where `event` is `paid` (the tenant takes `plan` from the catalog, status `active`), `lapsed` (status `pastDue`, `pastDueSince` now) or `cancelled` (status `pastDue` as well; the grace applies the same), signed with `BILLING_SECRET` in an `x-billing-signature` header (HMAC-SHA256 of the raw body, hex); a provider's own webhook format is adapted outside the app (a small relay), which is what keeps every provider's card handling out of it. A hosted checkout page per plan is configuration, `BILLING_CHECKOUT_<PLAN>` (a URL the landing page's Plans buttons and the Environment panel's **Upgrade** link go to, with `?environment=<slug>` appended), none set means the plan is not sold online.
- **The grace.** A tenant `pastDue` for 14 days is degraded to the `free` plan's caps by a sweep the host runs once an hour (status back to `active`, `plan.name` `free`, `degradedAt` recorded); the console shows past-due tenants with the date, and the Environment panel's banner counts down. Nothing is ever deleted by billing.
- **The tenant's export and deletion** are the Environment panel's two buttons above; a deletion request is carried out by a host admin on the console (the existing Delete, which moves the directory aside), never by itself.
