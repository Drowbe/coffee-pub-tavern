...
// Lobby and the new-space profile are unticked (the Lobby's words are the owner's by now). Apply sends what is ticked
// ({ modules, lobby, spaceDefaults }); even with nothing ticked it closes the offer. Not now only hides it here.
...
export const offerHasChoices = (offer) => Boolean(offer && ((offer.modules || []).length || offer.lobby || offer.spaceDefaults));
...
export function renderOffer(box, { templateName, offer, lobbyName = '', apply, later }) {
  ...
  box.innerHTML = `
    <div class="template-offer" role="group" aria-labelledby="${uid}">
      ...
      ${offer && (offer.lobby || offer.spaceDefaults) ? `<div class="offer-group"><span class="field-label">Also</span>${offer.lobby ? lobbyRow() : ''}${offer.spaceDefaults ? profileRow() : ''}</div>` : ''}
      <div class="row">
